...
const options = ['Lighting'];

const EditCourt = ({exitModal, courtData, clickedCourt, isCourtCard, modalOnAddNew, handleAddNewBtn}) => {
  
  const selectedCourt = !modalOnAddNew ? courtData.filter(item=> item.id === clickedCourt)[0] : '';
  console.log(clickedCourt);

  const [courtName, setCourtName] = useState(selectedCourt?.name);
  const [courtPrice, setCourtPrice] = useState(selectedCourt?.fee);
    ...
    <div className='add_court_container p-4'>
        ...
        <div className='row pt-5 justify-content-between'>
            <div className='col-md-6'>
                <p className='input_title secondary-light' onClick={()=> console.log(courtName)}> Name </p>
                <Input name="court-name" addCourtInput={addCourtInputStyles} value={courtName} onChange={handleChangeName}/>
            </div>
            <div className='col-md-6'>
                <p className='input_title secondary-light'> Court Type </p>
                <Select top={8} isAddCourt className='addCourt_select' optionList={selectedCourt?.features}/>
            </div>
        </div>
        <div className='row pt-2 justify-content-between'>
            ...
            <div className='col-md-6'>
                ...
                {console.log("FEATURES ====", selectedCourt?.features)}
                <Select 
                closeMenuOnSelect={false}
                // components={animatedComponents}
                defaultValue={[options.filter(item => selectedCourt?.features?.includes(item))]}
                isMulti
                options={options}
                // selectedOptions={selectedCourt?.activities}
        ...
        <div className='row pt-2 justify-content-between'>
            <div className='col-md-6'>
                <p className='input_title secondary-light' onClick={()=> console.log(courtPrice)}> Price </p>
                <Input top={8} addCourtInput={addCourtInputStyles} value={courtPrice} onChange={handleChangePrice}/>
